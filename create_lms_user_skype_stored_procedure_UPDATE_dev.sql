...
BEGIN    
    ...
    if (SELECT SKYPEID) != '' THEN
		UPDATE `repda_lms.profilequinte.com`.`cl_user_property` SET propertyValue = (SELECT SKYPEID) WHERE userId = (SELECT LMS_USER_ID);
	ELSE
		UPDATE `repda_lms.profilequinte.com`.`cl_user_property` SET propertyValue = (SELECT SKYPEID) WHERE userId = '';
	END IF;
